...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName("song").setDescription("Play song you want to play").setRequired(true)),
  cooldown: 3,
  permissions: [PermissionsBitField.Flags.Connect, PermissionsBitField.Flags.Speak],
  async execute(interaction: ChatInputCommandInteraction, input: string) {
    let argSongName = interaction.options.getString("song");
    if (!argSongName) argSongName = input;
    console.log("ID: "+interaction.user.id+" ACTION: cmdInteract.play CONTENT: "+argSongName);
